...
Uses Gemini (Google GenAI API) with FORCED function-calling for structured
output. Needs GEMINI_API_KEY in .env and `pip install google-genai`.

Switched from Anthropic to Gemini 2026-08-20, mid-build, before an
ANTHROPIC_API_KEY was ever added — nothing here was ported blind. The real
installed google-genai==2.19.0 client was introspected first (dir(client),
dir(client.models)), then a real forced-tool-call was made and the actual
response object inspected before this file was written:
    client.models.generate_content(model=..., contents=..., config=
        ...
        types.FunctionCallingConfig(mode="ANY")), system_instruction=...))
    -> resp.function_calls[0].name / .args   (.args is ALREADY a parsed
       dict, not a JSON string — no json.loads needed, confirmed by
       inspecting a real response, not assumed from docs)
gemini-2.5-pro and gemini-2.5-flash both 404 ("no longer available to new
users") on this key. gemini-3.1-pro-preview 429s (0 free-tier quota on
this key). gemini-3.7-flash is the one confirmed to actually work end to
end with a real forced tool call — see MODEL below.
...
"values" mapping, so Gemini has no listed option to match against and
...
RELIABILITY: gemini-3.7-flash returned 503 "high demand" intermittently
and reproducibly on the full 223-code catalog forced-tool-call — roughly
every other real call during testing, on two different API keys/projects,
with no correlation to daily quota (confirmed via the raw 429 quota body
on one key, which cited a plain 20-requests/day project cap, separate
from these 503s seen on a freshly-quota'd key too). Not a hard ceiling —
successive identical calls succeeded and failed unpredictably. _with_retry
below (mirrors capture.py's) handles it with exponential backoff.
...
def extract(transcript, doc_text=""):
    ...
    # resp.function_calls[0].args is ALREADY a parsed dict (confirmed via a
    # real call, see the module docstring) — no json.loads needed.
    parsed = resp.function_calls[0].args
